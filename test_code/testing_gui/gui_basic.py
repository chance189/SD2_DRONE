...
class Thread(QThread):
    ...
    def setup_gstream(self):
        ...
        self.from_video_slow_fps = "gst-launch-1.0 filesrc location=/home/chance/Desktop/SD2_DRONE/deepstream_proof_using_config/sample_1080p_h264.mp4 ! qtdemux ! h264parse !  nvv4l2decoder ! m.sink_0 nvstreammux name=m batch-size=1 width=1280 height=720 gpu-id=0 nvbuf-memory-type=0 ! nvinfer config-file-path=/home/chance/Desktop/SD2_DRONE/deepstream_proof_using_config/config_infer_primary_yoloV3_tiny.txt batch-size=1 unique-id=1 ! nvvideoconvert ! nvdsosd ! nvegltransform ! nveglglessink"
        self.moreFUCKINGDEBUGGIN = "gst-launch-1.0 v4l2src device="/dev/video0" ! "video/x-raw, width=1280, height=720, format=(string)UYVY" ! nvvidconv ! 'video/x-raw(memory:NVMM), format=(string)I420' ! nvv4l2h264enc maxperf-enable=1 bitrate=8000000 ! 'video/x-h264, stream-format=(string)byte-stream' ! h264parse !  nvv4l2decoder ! m.sink_0 nvstreammux name=m batch-size=1 width=1280 height=720 gpu-id=0 nvbuf-memory-type=0 ! nvinfer config-file-path=/home/chance/Desktop/SD2_DRONE/deepstream_proof_using_config/config_infer_primary_yoloV3_tiny.txt batch-size=1 unique-id=1 ! queue ! nvvideoconvert ! nvdsosd ! nvegltransform ! nveglglessink"
